...
import type { SkladRequest, RequestStatus, RequestItem, TelegramEvent, RequestChain } from '../types';
import {
...
export default function RequestDetailPage() {
  ...
  const actionConfig: Partial<Record<RequestStatus, { label: string; className: string; icon: React.ElementType }>> = {
    vydano: { label: 'Выдать полностью ✓', className: 'flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-xl hover:bg-green-700 transition-colors font-medium', icon: CheckCircle2 },
    sklad_partial: { label: 'Частично выдать', className: 'flex items-center gap-2 px-4 py-2 bg-orange-500 text-white rounded-xl hover:bg-orange-600 transition-colors font-medium', icon: Package },
  ...
  const isCompleted = request.status === 'vydano' || request.status === 'otkloneno';
        ...
        <div className="bg-blue-50 border-2 border-blue-200 rounded-2xl p-5">
          ...
          <div className="flex flex-wrap gap-2">
            {nextStatuses.map(status => {
              ...
              const Icon = cfg.icon;
              return (
                <button key={status} onClick={() => updateStatus(status)} disabled={actionLoading}
